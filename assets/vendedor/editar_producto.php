...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_SESSION['rol']) && $_SESSION['rol'] === 'admin') {
        $id_vendedor = 1; // administrador
    } else {
        $id_vendedor = $_SESSION['id']; // id del vendedor logueado
    }
    ...
    $stmt = $conn->prepare("INSERT INTO productos 
        (id_vendedor, nombre, descripcion, descripcion_larga, precio, imagen, categoria, stock, material, color, origen, estilo, garantia) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param(
        "isssdssisssss",
        $id_vendedor,
    ...
    if (isset($_SESSION['rol']) && $_SESSION['rol'] === 'admin') {
        ...
        header("Location: producto_vendedor.php");
    }
    exit();
}
?>
...
<head>
    <meta charset="UTF-8">
    <title>Agregar Producto | Tinkuy</title>
